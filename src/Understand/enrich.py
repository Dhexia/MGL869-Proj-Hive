...
def aggregate_metrics_by_file(enriched_df: pd.DataFrame) -> pd.DataFrame:
    """
    Aggregates metrics by FileName, removes empty rows, and drops the Kind column.
    
    Parameters:
        enriched_df (pd.DataFrame): The DataFrame containing enriched metrics.
    
    Returns:
        pd.DataFrame: Aggregated metrics by FileName with Kind column removed.
    """
    # Aggregation rules
    aggregation_rules = {
        "CountClassBase": "sum",
    ...
    # Optionally, you can keep this line here if you need Kind before the drop
    # aggregated_metrics["Kind"] = "File"
